...
input_urls = [input('Enter the Search Url: ')]
bucket_name = 'marketplace-scanner'
folder_name = f"Meesho/Meesho_{search_string}_{timestamp}"


uids = generate_uids(count=len(input_urls), length=15)

s3_client = boto3.client('s3', aws_access_key_id=aws_access_key, aws_secret_access_key=aws_secret_key,
                         region_name='ap-south-1')
data_dict = {}

for uid, url in zip(uids, input_urls):
    data_dict[uid] = {"Product_url": url}

for uid, value in data_dict.items():
    print(f"Processing Product ID: {uid}, URL: {value['Product_url']}")
...
s3_client.upload_fileobj(io.BytesIO(updated_json_string.encode('utf-8')), bucket_name, f"{folder_name}/{search_string}_details.json")
...
s3_client.upload_fileobj(reviews_json_buffer_prod, bucket_name, f"{folder_name}/{search_string}_product_comments.json")
...
s3_client.upload_fileobj(reviews_json_buffer_seller, bucket_name,f"{folder_name}/{search_string}_seller_comments.json")
...
s3_client.upload_fileobj(predictions_json_buffer, bucket_name, f"{folder_name}/{search_string}_product_review_prediction.json")
...
s3_client.upload_fileobj(predictions_json_buffer1, bucket_name, f"{folder_name}/{search_string}_seller_review_prediction.json")
